...
已上线的 P0/P1/P1.5/F0 构成 **M1**(收入→权益→用量→欠费执行,已闭环)。下一里程碑 **M2 = P2 + P3 + F1**(自助化与可对账)。

🎯 **开工前先读设计文档的 [§4.5 下一里程碑](../stripe-billing-integration-plan.md#45-下一里程碑m2--p2--p3--f1--开工前必须先解决的问题)** —— 那里记录了 4 个**照着原计划直接开工会撞上的阻塞项**:

| # | 阻塞项 | 性质 |
|---|---|---|
| 1 | **退款执行权归属**:架构说 billing-service 不碰 Stripe,accounts 注销决策却说退款由 billing-service 执行,而 P2 又写 accounts 直接调 refund —— 三处互相矛盾 | ⛔ 纯决策,不拍板无法开工 |
| 2 | **P2 依赖 P3**:退款判定需要 `GET /v1/usage/window`,但它排在 P3 —— 当前排序是反的 | ⛔ 需调顺序 |
| 3 | **P2「注销联动」与 accounts 侧已锁的自助注销重复**:两边在规划同一个功能 | ⚠️ 需归口,避免两份漂移的设计 |
| 4 | **`reconcile` 命名冲突**:本仓已有 `/v1/jobs/reconcile`(实为用量补算),P3 要加语义完全不同的 `reconcile-stripe` | ⚠️ 需改名 |

建议执行顺序与前置条件(付费套餐录入、F0 凭据接线、sandbox 回归)同见 §4.5。

**并行的技术欠账**(不阻塞 M2,但建议顺手清):schema 所有权注释(§四.2,低成本)、PGMQ 队列去留(§四.1,随 P3 决策)、throttle 预警的 console/邮件一半(§四.5)。
